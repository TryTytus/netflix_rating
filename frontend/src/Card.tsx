...
function Card({title, poster_url, rating, vote_count}) {
    ...
    const getFormatedVotes = (vote_count: number): string =>  {
        if (vote_count < 1000)
        {
            return '>1k'
        }
    ...
    useEffect(() => {
        ...
        card.current?.addEventListener("m", handle)

        return () => {
            card?.current?.removeEventListener("mouseenter", handle)
        }
    }, [])

    return (
    <>
        <div ref={card} className={`${width} shrink-0 relative`}>
            <img className={`h-[250px] ${width} object-cover`} src={poster_url} />

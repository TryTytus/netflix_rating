...
function Card({title, poster_url, rating, vote_count}) {
    ...
    useEffect(() => {
        ...
        const handleEnter = () => {
            ...
            console.log("heyy!!")
            setTimeout(() => {
                if (isHovered.current && Date.now() - lastTimeChangedMs.current > 2000)
                {
                    console.log("OPEN!!!!")
                    openPopupCard()
                }
    ...
    <>  
        <div ref={card} className={` shrink-0 w-[200px] relative overflow-y-visible`}>
            {(isOpened) && createPortal((<BigCard top={getTop()} left={getLeft()} {...{title, poster_url, rating, vote_count}} />), document.body)}
